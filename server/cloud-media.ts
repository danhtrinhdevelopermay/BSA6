...
class CloudMediaService {
  ...
  async processMediaUrls(mediaUrls: Array<{ type: string; url: string; thumbnailUrl?: string }>): Promise<Array<{ type: string; url: string; thumbnailUrl?: string }>> {
    console.log('Processing media URLs:', mediaUrls.length, 'items');
    
    // If no API key is configured, skip cloud upload and return original URLs
    if (!this.imgbbApiKey) {
      console.log('IMGBB_API_KEY not configured, skipping cloud upload');
      return mediaUrls;
    }
    
    console.log('IMGBB_API_KEY configured, proceeding with cloud upload');
    const optimizedUrls = [];
    
    for (const media of mediaUrls) {
      console.log('Processing media item:', media.type, 'URL length:', media.url.length);
      ...
      if (media.url.startsWith('data:')) {
        try {
          if (media.type === 'image') {
            ...
            console.log('Image uploaded successfully:', cloudResult.url);
            optimizedUrls.push({
              type: media.type,
              url: cloudResult.url,
            ...
            console.log('Keeping video URL as-is');
        ...
        console.log('URL already optimized, keeping as-is');
